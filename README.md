...
> **검증 기준 버전**: 이 저장소의 출력은 `torch 2.4.1 / transformers 4.44.2 /
> sentence-transformers 3.0.1 / numpy 1.26.4`에서 검증했습니다. `requirements.txt`는
> 독자 환경 유연성을 위해 하한(`>=`)으로 표기하며, 상위 호환을 허용합니다. 다만
> **정확한 출력 문자열이 약속된 실습(예: 12-3 RAG의 "16일")은 그리디 디코딩 재현성을 위해
> 해당 노트북 [준비] 셀에서 `transformers==4.44.2`를 인라인으로 고정**합니다. 최신 환경에서
> 결과가 다르면 각 [준비] 셀의 버전 출력을 확인하세요.
> 12-3의 생성 모델은 `Qwen/Qwen2.5-3B-Instruct`입니다(소형 모델의 표 독해 오류를 피하기 위한 선택).

## 노트북 읽는 법

- `[셀 n] … 📖` — 책 지면 코드에 **대응**하는 셀. 지면은 핵심을 발췌·정제해 싣고(함수·변수명은 지면 가독성 기준이라 다를 수 있음), 전체 구현은 이 저장소에 있으며, 실행하면 **같은 출력·수치가 재현**된다
